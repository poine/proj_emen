...
def compute_and_set_scale2(anims, mode='each'):
    if mode == 'each':
        for _a in anims:
...
def animate_multi(fig, axes, drones, targets, names, t0=0., t1=None, dt=0.1, xlim=(-200, 200), ylim=(-200, 200)):
    ...
    compute_and_set_scale2(anims, mode='same')
    ...
    def _init():
        ...
        for _a in anims:
            foo+=_a.init()
        return foo  + _line_targets#+ _line_drones
    ...
    def _animate(i):
        ...
        return foo + _line_targets# + _line_drones + + _status_texts
